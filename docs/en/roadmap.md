...
P2.1 was accepted on 2026-07-24. It delivers the physical Knowledge module, fail-closed enablement and health, private Worker deployment, six scoped immutable ingestion tables, safe inline/upload/web capture, deterministic five-format parsing and chunking, persisted leased execution, restart recovery, bounded retry, honest cancellation, inspection, resynchronization, tombstone, audit, and low-cardinality telemetry. Candidate PR #10 and the resulting `main` run both passed the complete CI suite, including the isolated five-source and restart-recovery Compose workflow. P2 remains in progress: Knowledge stays disabled by default and contract-only, while immutable indices and Retrieval Lab begin in P2.2.
