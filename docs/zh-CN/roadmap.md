...
P2.1 正在实施。P2.1a 建立物理 Knowledge 模块、默认拒绝的启用与 Health、Worker 私有部署边界，以及版本化 Parser 候选语料/决策。P2.1b 增加六张带 Scope 的摄取表、由数据库强制执行的不可变血缘，以及必须接收 Tenant/Workspace Scope 的 Repository。P2.1c 闭合 Base 与非 Web Source 的编写流程，包括不可变 Revision、排队 Job、Audit、No-op 识别、内容读取和 Tombstone。Knowledge 仍默认关闭；Web Capture、处理、检索、Live API 与产品页面仍需等待后续检查点和 P2 Exit Gate。
